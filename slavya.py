...
@db.message_handler(lambda message: message.chat.type == 'private', content_types=['photo'])
async def handle_predlojka(message: types.message):
    a = message.photo[-1].file_id
    await bot.send_photo(1270866514, a, F'От [{message.from_user.first_name}](tg://user?id={message.from_user.id})', parse_mode='markdown')


@db.message_handler(commands=['timetable'])
async def handle_count(message: types.Message):
    if message.chat.id not in [-1001283223823,-413998930]:
         return
    tz = pytz.timezone('Europe/Moscow')
    m = datetime.now(tz)
    day = m.weekday()
    if message.chat.id in [-1001283223823]:
        if day == 0:
            await message.reply('''
⚠️Данное расписание 100% актуально для *ГРУППЫ М1О-109Б-20*⚠️

Расписание на понедельник:
9:00-10:30 - физика
13:00-14:30 - теоритическая механика

Расписание на вторник:
14:45-16:15 - химия
16:30-18:00 - культурология
...
Расписание на среду:
9:00-10:30 - физика
10:45-12:15 - теоретическая механика
13:00-14:30/18:00-? - лабораторная работа по химии/материаловедению (чередуются каждую неделю)
14:45-16:15 - английский язык *⭕️(Группа А)⭕️*
...
9:00-10:30 - лабораторная работа по физике
14:45-18:00 - математический анализ
...
*🛑Время МОСКОВСКОЕ🛑*''', parse_mode='markdown')
    if message.chat.id in [-413998930]:
        if day == 0:
            await message.reply('''
⚠️Данное расписание 100% актуально для *ГРУППЫ М3О-103Б-20*⚠️

Расписание на понедельник:
8:00-10:30 - физика (Тимс)
10:45-12:15 - АЯП

Расписание на вторник:
13:00-15:15 - основы прикладной гидро и аеродинамики(ЛМС) *❗️❗️❗️РАЗ В ДВЕ НЕДЕЛИ❗️❗️❗️*
16:30-18:00 - культурология (те кто ходят ок, те кто нет, тоже не проблем)
...
Расписание на среду:
9:00-10:30 - физика (Тимс)
10:45-12:15 - электротехника (ЛМС)
13:00-14:30 - инженерная графика (Тимс)
...
Расписание на четверг:
14:45-18:00 - математический анализ (Тимс)
...
Расписание на пятницу:
10:00-11:30 - инженерная графика (Тимс)
13:00-14:30 - основы прикладной гидро и аеродинамики(ЛМС)
